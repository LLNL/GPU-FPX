#EXEs=$(find CUDA/ -type f -executable) # find all executable paths
EXEs=$(find CUDA/ -name "*.exe") # find all executable paths
...
for exe in ${EXEs}
do
        e_file=${exe##*/} # get the name of executable [[Bash -- get the last substring for a separator]]
#       mv CUDA/${e_file} CUDA/${e_file}.exe
#       mkdir CUDA/${e_file}
#       mv CUDA/${e_file}.exe CUDA/${e_file}
        size=${#e_file} #[[Bash -- get the length of a string]]
